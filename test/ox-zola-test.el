...
(ert-deftest ox-zola-test-updated-field ()
...
(ert-deftest ox-zola-test-template-field ()
...
(ert-deftest ox-zola-test-hugo-unaffected ()
...
(ert-deftest ox-zola-test-single-frontmatter ()
...
(ert-deftest ox-zola-test-no-keywords ()
...
(ert-deftest ox-zola-test-nil-base-dir-no-crash ()
...
(ert-deftest ox-zola-test-macro-properties ()
  "Test export with :PROPERTIES:, #+MACRO, custom front matter."
  (let ((org-inhibit-startup t))
    (with-temp-buffer
      (insert (ox-zola-test--read-fixture "with-macro-properties.org"))
      (org-mode)
      (condition-case err
          (let ((export-buf (ox-zola-export-as-md)))
            (unwind-protect
...
(ert-deftest ox-zola-test-closed-subtree ()
  "Test export with CLOSED, :PROPERTIES:, src blocks, id: links."
  (let ((org-inhibit-startup t)
        (org-export-with-broken-links t))
    (with-temp-buffer
      (insert (ox-zola-test--read-fixture "with-closed-subtree.org"))
      (org-mode)
      (condition-case err
          (let ((export-buf (ox-zola-export-as-md)))
            (unwind-protect
...
;;; File export (hugo-only.org fixture)

(ert-deftest ox-zola-test-file-export-hugo-only ()
  "Test file export with HUGO_* keywords only."
  (let* ((org-inhibit-startup t)
         (temp-dir (make-temp-file "ox-zola-test-" t)))
    (unwind-protect
        (with-temp-buffer
          (insert (ox-zola-test--read-fixture "hugo-only.org"))
          (goto-char (point-min))
...
(defvar ox-zola-test-repo-root
  (expand-file-name ".."
                    (file-name-directory (or load-file-name buffer-file-name)))
  "Repository root directory.")

(defvar ox-zola-test-build-dir
  (expand-file-name "build/test-output" ox-zola-test-repo-root)
  "Build directory for E2E test artifacts.")

(defun ox-zola-test--clean-build-dir ()
  "Ensure the build directory exists."
  (unless (file-directory-p ox-zola-test-build-dir)
    (make-directory ox-zola-test-build-dir t)))

(ert-deftest ox-zola-test-e2e-zola-keywords-output-path ()
  "E2E: ZOLA_* keywords produce correct output path."
  (let* ((org-inhibit-startup t)
         (fixture-file (expand-file-name "e2e-zola-keywords.org"
                                         ox-zola-test-data-dir))
         (base-dir (expand-file-name "zola-keywords"
                                     ox-zola-test-build-dir))
         (expected-dir (expand-file-name "content/posts" base-dir)))
    (ox-zola-test--clean-build-dir)
...
(ert-deftest ox-zola-test-e2e-hugo-keywords-output-path ()
...
(ert-deftest ox-zola-test-no-relref-in-output ()
  "Verify no relref appears in exported Markdown."
  (let ((org-inhibit-startup t))
    (with-temp-buffer
      (insert (ox-zola-test--read-fixture "with-links.org"))
      (org-mode)
      (let ((export-buf (ox-zola-export-as-md)))
        (unwind-protect
            (with-current-buffer export-buf
...
(ert-deftest ox-zola-test-same-file-heading-link ()
...
(ert-deftest ox-zola-test-custom-id-link ()
...
(ert-deftest ox-zola-test-heading-title-link ()
...
(ert-deftest ox-zola-test-external-url-unchanged ()
...
(ert-deftest ox-zola-test-file-link-no-relref ()
...
(ert-deftest ox-zola-test-file-link-empty-section ()
  "Verify file: links with empty section produce @/post.md not @//post.md."
  (let ((org-inhibit-startup t)
        (ox-zola-section ""))  ; Override default to empty
    (with-temp-buffer
      (insert "#+title: Test\n\n[[file:other.org][other post]]\n")
      (org-mode)
      (let ((export-buf (ox-zola-export-as-md)))
...
(ert-deftest ox-zola-test-file-link-with-anchor ()
...
(ert-deftest ox-zola-test-cross-section-link ()
...
(ert-deftest ox-zola-test-non-paired-shortcode ()
...
(ert-deftest ox-zola-test-shortcode-multiple-args ()
...
(ert-deftest ox-zola-test-shortcode-body-as-arg ()
...
(ert-deftest ox-zola-test-shortcode-empty-args ()
...
;;; Semantic blocks (description, details, aside)

(ert-deftest ox-zola-test-description-block ()
...
(ert-deftest ox-zola-test-details-block ()
...
(ert-deftest ox-zola-test-details-block-no-summary ()
...
(ert-deftest ox-zola-test-details-block-sep-at-start ()
...
(ert-deftest ox-zola-test-details-block-whitespace-sep ()
...
(ert-deftest ox-zola-test-details-block-midline-dash ()
